...
/// right. Counts are formatted compactly (12400 → "12.4K").
///
/// Most wallpapers won't have author info attached — when [authorName] is
/// empty we don't fabricate a name. Instead we show a quiet "WALLR
/// Community" label and drop the handle line entirely, so the row still
/// reads as intentional rather than like a broken/placeholder state.
class AuthorStatsRow extends StatelessWidget {
  final String authorName;
  final String authorHandle;
  final String authorAvatarUrl;
  ...
  @override
  Widget build(BuildContext context) {
    final hasName = authorName.isNotEmpty;
    final displayName = hasName ? authorName : 'WALLR Community';
            ...
            children: [
              Text(
                ...
                style: hasName
                    ...
                    : AppTextStyles.bodySmMuted,
              ),
              // Only show a handle when we actually have a real author —
              // never pair it with the generic fallback label.
              if (hasName && authorHandle.isNotEmpty)
                Text(
                  authorHandle.startsWith('@')
                      ? authorHandle
                      : '@$authorHandle',
